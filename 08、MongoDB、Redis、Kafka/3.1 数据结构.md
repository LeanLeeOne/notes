...
<table cellspacing="0" cellpadding="0">
	...
	<tbody>
		...
		<tr><td>Hash</td><td>字典</td><td><u>键值对</u>集合，<br/>即编程语言中的Map类型</td><td>适合存储对象，并且可以像数据库中update一个属性一样只修改某一项属性值(Memcached中需要取出整个字符串反序列化成对象修改完再序列化存回去)</td><td>1、存储、读取、修改用户等<span style=background:#c2e2ff>对象</span>的属性</td></tr>
...
为了保证性能（节点数量与散列表的大小为`1:1`时性能最好），每次向**Dictionary**中增加<u>键值对</u>时，都会对`dict.ht[0]`的节点数量与散列表大小的比率（Ratio）作比较，当Ratio大于某些阈值时就会触发扩容。

扩容时，将`dict.ht[0]`上的<u>键值对</u>逐个`Rehash`到`dict.ht[1]`上，而该`dict.ht[0]`的`table[rehashidx]`会指向`Null`，`dict.reashidx`会加1，`Rehash`完毕后会交换这两个`dict.ht`的角色。
