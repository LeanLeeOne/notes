...
当数据增长到一定的规模，**OLAP**就不适合放在RDBMS中进行，这时往往会将**OLAP**剥离到专门的系统中，让RDBMS专注**OLTP**、减小压力。而这一专门的系统被叫做数据仓库。
...
> 事实表中一条记录所表达的业务细节程度被称为[粒度](https://help.aliyun.com/zh/maxcompute/getting-started/data-warehouse-detail-layer)。粒度通常可以通过两种方式来表述：
>
> 1. 维度组合所表示的细节程度。
> 2. 所表示的具体业务含义。
...
> DWD以业务过程为驱动，DWM、DWS、DWT、ADS以需求为驱动。
...
数据汇总层，Data Warehouse Summary，DWS，是在DWM的基础上，整合/汇总成多个主题，用于业务查询、OLAP、数据分发。DWS是面向分析对象的主题聚集建模。

DWS的每个主题通常为一张宽表，即，表往往数量相对较少、字段较多，一张表会涵盖比较多的业务内容。DWS的一个表至少会对应一个派生指标。

#### 数据主题层

数据主题层，Data Warehouse Topic，DWT。

DW一般只分为DWD、DWS，[但也有采用](https://blog.csdn.net/wjt199866/article/details/115184169)DWD、DWS、DWT这种分层方式，这种方式：

- 将之前的DWM/DWB去掉，由DWS进行全部聚合操作。
- 同时，将DWS中按主题汇聚指标/建表的职责，提取为DWT。

也采用有DWD、DWM、DWS分层方式的公司。
