...
[字符指](https://baike.baidu.com/item/字符/4768913)类字形单位或符号，包括字母、数字、运算符号、标点符号和其他符号，以及一些功能性符号。

> 字符也可分为可打印字符和非可打印的控制字符。

字符集（Charset）就是一套字符集合，常见的字符集有**ASCII**、**Unicode**、**GB2312**等。
...
>
> [在UTF-8中](https://www.zhihu.com/question/274104168/answer/2425484354)，数字、英文等字符仅占`1`字节，中文占`3`字节，emoji、繁体字等字符占`4`字节，**MySQL**中的**utf8**仅支持`1~3`个字节，不是完整的`UTF-8`，2010年推出的**utf8mb4**才是完整的**UTF-8**。
>
...
之后，国家又推出[GB 13000.1-93](https://zh.m.wikipedia.org/zh-hans/GB_13000)、**GB 13000-2010**等字符集。
...
1. `RegularEnumSet`使用一个`long`类型的变量作为<u>位向量</u>。
2. `JumboEnumSet`使用一个`long`类型的数组存储<u>位向量</u>。
