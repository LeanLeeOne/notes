...
除了存储引擎层面的缓存，**MySQL**还提供全局层面/服务器层的查询缓存（只针对`SELECT`）。
...
当表发生变化时，与该表相关的缓存都会失效。当缓存空间不足时，会淘汰某些缓存。

缓存能加速读取，尤其是那些读多写少（`10`读`1`写）或包含负责计算（聚合）的SQL，但缓存也有一定的限制：

- 缓存开启后，会成为服务器层的资源竞争单点，可能会导致服务器僵死。
- 缓存的读写都有开销，如果命中率低，缓存的开销可能反而高于收益。

不是每条查询都能从缓存受益或受害，所以，是否开启缓存，要从全局出发，综合考虑。