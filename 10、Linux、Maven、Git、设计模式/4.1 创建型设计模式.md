...
单例有[八种](https://www.cnblogs.com/javastack/p/12579198.html)写法，
...
与“饿汉模式Ⅰ”几乎没有区别，只是将“直接对静态变量赋值”改为“在静态代码块中赋值”，所以有人认为单例只有[七种](https://blog.csdn.net/itachi85/article/details/50510124)。
...
首次加载`Singleton`时不会初始化`instance`，只有第一次调用`getInstance()`时，JVM才会加载`SingletonHolder`，这时才会初始化`instance`。

<span style=background:#d4fe7f>推荐使用</span>。
...
前几种写法如果要杜绝单例对象被反序列化时重新生成对象，就必须加入如下方法：
