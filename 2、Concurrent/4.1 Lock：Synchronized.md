...
其中，对象头包括：<span style=background:#d4fe7f>Mark Word</span>、类型指针。<span style=background:#d4fe7f>Mark Word</span>用来保存锁标识（bit fields，类型标识；tag bits，<span style=background:#c9ccff>状态标识</span>）。
