...
`SELECT COUNT(字段)`会忽略值为`NULL`的记录，并且该字段如果有<span style=background:#c2e2ff>索引</span>的话，查询速度能大幅提升，但如果该字段也出现在了`WHERE`中，索引将失效，**Optimizer**会改为全表扫描！但对于`SELECT COUNT(主键)`，由于是聚族索引，记录比较大，IO次数会多，查询速度会慢一些。
