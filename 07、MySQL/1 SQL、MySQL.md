...
>
> **InnoDB**的表空间不只存储了索引和数据，还保存了<span style=background:#f8d2ff>Undo Log</span>、插入缓冲、双写缓冲以及其它内部数据结构。
>
> 插入缓冲（Insert Buffer）在新版的**InnoDB**中叫做变更缓冲（Change Buffer），用于减少由要更新的行的非唯一索引不在内存中所引起的随机IO。
