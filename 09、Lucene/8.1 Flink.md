...
- Event Time，事件时间，由开发者提指定，通常指定数据的最原始的创建时间。**Flink**会根据Event Time生成Watermark来进行关窗，即只有当Watermark大于关窗时间时，才会触发窗口的结束，才会输出结果。如果一直没有数据来触发窗口结束，则该窗口就无法输出结果。单个subtask的Watermark是递增的，多个subtask或者多个源表的Watermark取最小值。可以通过窗口结束。
