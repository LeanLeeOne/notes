...
<table cellspacing="0" cellpadding="0">
	<thead>
		<tr>
			<th style="width: 75px; min-width: 75px;">类型</th>
	...
	<tbody>
		<tr><td>String</td><td>字符串</td><td>二进制安全</td><td>可以包含任何数据，比如jpg图片或者序列化的对象，<br/>一个键最大能存储512M</td><td>1、简单的缓存<br/>2、计数器</td></tr>
...
针对以上5种数据类型，**Redis**设计了[不同的数据结构](https://redisbook.readthedocs.io/en/latest/index.html)。
...
1. 实现字符串对象。
2. 做<span style=background:#b3b3b3>char\*</span>的替代品：减少了字符串追加操作产生的内存重分配次数，简化了字符串的长度计算。
3. 二进制安全。
...
**Zip List**要比**Linked List**、**Dictionary**省内存，所以当元素较少时，**Redis**会用**Zip List**，当键值对的值超出阈值时，或者节点数量超出阈值时，**List**才会将**Zip List**转为**Linked List**，**Hash**才会将**Zip List**转为**Dictionary**。
...
**Int Set**中的整数可由16位升级为32位、32位升级位64位，整数最终几位，取决于位数最大的那个整数，升级会引起整个**Int Set**的内存的重新分配。

**Int Set**使用二分查找算法，来判断整数在集合中的位置（数组的索引）。
...
**Skip Lists**基于多指针有序链表，查找数据时，从**Skip Lists**的上层指针开始查找，找到对应区间后再向下层指针查找。
...
1. 插入速度快，因为不需要进行**旋转**来维护平衡性。
2. 支持无锁操作。
3. 容易实现。

**Redis**中的**Skip Lists**在通用设计的基础上增加了：
