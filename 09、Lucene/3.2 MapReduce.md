...
但在真正写磁盘之前，Map会将生产的<u>键值对</u>分到不同的**Partition**中，并且每个**Partition**会有一个后台线程在内存中对<u>键值对</u>进行排序。
...
如果**Partition**文件数量很多，那么Reduce会进行多次合并，直到小于阈值（默认为`10`）。

合并的同时会进行排序。

Reduce不会将**Partition**文件合并为一整个文件，而是在内存中将进行最终的合并。
...
**MapReduce**会在Map、Reduce分别进行`1`次合并，合并的同时会进行归并排序。

Map的`Partition、Sort and Spill to Disk`的后台线程[采用的是](https://www.cnblogs.com/bigband/p/13518507.html)快速排序。
...
如[下图](https://blog.csdn.net/iteye_14970/article/details/82639055)所示，SequenceFile由[文件头]()和记录组成，是可切分的。记录共有`3`种格式：未压缩、记录级压缩和块级压缩。
...
ORCFile支持多种压缩算法，最常用的是Zlib和Snappy，但两者都是基于需要读取整个文件内容的流式压缩的算法，[不支持文件分割](https://zhuanlan.zhihu.com/p/257917645)，对此，ORCFile采用分块，即，在写入时限定文件大小的做法。
