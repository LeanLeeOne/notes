...
如上表所示，对于<span style=background:#f8d2ff>表锁</span>和<span style=background:#c9ccff>行锁</span>来说，只有**共享**<span style=background:#f8d2ff>表锁</span>和**共享**<span style=background:#c9ccff>行锁</span>可以共存。

### 作用

<span style=background:#c2e2ff>意向锁</span>用于标识<u>当前事务</u>有意向表中的<span style=background:#c9ccff>行</span>施加**S锁**还是**X锁**，这样<u>后到的事务</u>如果需要加<span style=background:#f8d2ff>表锁</span>时，就无需通过<u>扫描每一行</u>来判断是否有**独占**<span style=background:#c9ccff>行锁</span>，进而决定是否等待<u>当前事务</u>释放<span style=background:#c9ccff>行锁</span>，还是立即执行。
