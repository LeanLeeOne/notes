...
>
> 添加Partition时，可以为该Partition单独指定Bucket的数量，但，Partition的Bucket数量一旦指定，不可更改，所以，在确定Bucket数量时，需要预先考虑集群扩容的情况。
>
...
**Doris**取Key的前`36Byte`作为前缀索引，但遇到`VARCHAR`类型的字段时，前缀索引会直接截断。
