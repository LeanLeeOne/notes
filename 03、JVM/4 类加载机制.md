...
- 为<span style=background:#ffb8b8>静态变量</span>分配内存空间，并赋以默认值，例如：
- 对于<span style=background:#ffb8b8>静态变量</span>`public static int value = 123`，会被赋予默认值`0`；
  - 而对于常量`public static final int value = 123`，则会被赋予值`123`。
...
- 只有当<span style=background:#c2e2ff>类被使用</span>时，才会触发类的初始化。
...
2. 显然，<span style=background:#c2e2ff>对象实例化</span>不是<span style=background:#c9ccff>类加载</span>的一个阶段，<span style=background:#c2e2ff>对象实例化</span>发生于<span style=background:#c9ccff>类加载</span>之后，不要混淆。
   ...
   2. <span style=background:#ffb8b8>静态变量</span>在<span style=background:#c9ccff>类加载</span>的**Preparation**段分配空间（在”**方法区/元空间**“中），**实例变量**在<span style=background:#c2e2ff>对象实例化</span>时分配空间（在<span style=background:#c9ccff>堆</span>中）。
...
1. 遇到`new`、`getstatic`、`putstatic`、`invokestatic`等字节码指令，且类没有进行过初始化时。这4条指令具体对应以下场景：
   1. `new`，实例化对象时。
...
1. 自定义<span style=background:#c9ccff>类加载器</span>，需重写`classLoader.findClass(String)`。
2. 重写`ClassLoader.loadClass(String)`可打破<span style=background:#c2e2ff>父类委托机制</span>。
...
<span style=background:#c2e2ff>父类委托机制</span>不是一个强制性的规范，如**Tomcat**，以及涉及**SPI**的地方（JDBC，JNDI，JAXB）出于需要，就没有遵循这一机制。
...
**SPI**中的<span style=background:#c2e2ff>实现类</span>会在`jar/META-INF/services/`中创建一个一服务接口的<span style=background:#c2e2ff>全限定类名</span>命名的文件，用于<span style=background:#c2e2ff>实现类</span>的装载。

[文章的评论区](https://blog.csdn.net/yangcheng33/article/details/52631940)提到，通过`ClassLoader.getSystemClassLoader()`能在单机环境中获取**Application Class Loader**，即**SPI**不一定非要依赖**TCCL**。但需要注意的是，`ClassLoader.getSystemClassLoader()`如果在服务器环境中，获取到的就会时服务器自定义的<span style=background:#c9ccff>类加载器</span>。这可能也是**Tomcat**中的**Common Class Loader**也是使用**TCCL**，而非`ClassLoader.getSystemClassLoader()`来获取**Webapp Class Loader**的原因。
