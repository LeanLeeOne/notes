...
为了保证性能（<u>节点数量与散列表的大小</u>为`1:1`时性能最好），每次向**Dictionary**中增加<u>键值对</u>时，都会对`dict.ht[0]`的<u>节点数量与散列表大小</u>的比率（Ratio）作比较，当Ratio大于某些阈值时就会触发扩容。

扩容时，将`dict.ht[0]`上的<u>键值对</u>逐个`Rehash`到`dict.ht[1]`上，而该`dict.ht[0]`的`table[rehashidx]`会指向`Null`，`dict.reashidx`会加1，直到`dict.reashidx`等于`dict.ht[0]`的长度，`Rehash`完毕，然后会交换这两个`dict.ht`的角色。
