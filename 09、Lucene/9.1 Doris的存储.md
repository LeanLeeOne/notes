...
| 导入方式     | 使用方式                                                     | 支持文件格式            | 数据源                                |
| ------------ | ------------------------------------------------------------ | ----------------------- | ------------------------------------- |
| Spark Load   | [通过Spark导入外部数据](https://doris.apache.org/zh-CN/docs/dev/data-operate/import/import-way/spark-load-manual) |                         |                                       |
| Broker Load  | [通过Broker导入外部存储数据](https://doris.apache.org/zh-CN/docs/dev/data-operate/import/import-way/broker-load-manual) | csv、gzip、parquet、orc | S3、HDFS                              |
| Stream Load  | [流式导入数据(本地文件及内存数据)](https://doris.apache.org/zh-CN/docs/dev/data-operate/import/import-way/stream-load-manual) | csv、json、parquet、orc | 本地文件及内存数据                    |
| Routine Load | [导入Kafka数据](https://doris.apache.org/zh-CN/docs/dev/data-operate/import/import-way/routine-load-manual) | csv、json               | Kafka                                 |
| Insert Into  | [外部表通过INSERT方式导入数据](https://doris.apache.org/zh-CN/docs/dev/data-operate/import/import-way/insert-into-manual) |                         | MySQL、PostgreSQL、Oracle、SQL Server |
| S3 Load      | [S3协议的对象存储数据导入](https://doris.apache.org/zh-CN/docs/dev/data-operate/import/import-way/s3-load-manual) |                         |                                       |
| MySQL Load   | [MySQL客户端导入本地数据](https://doris.apache.org/zh-CN/docs/dev/data-operate/import/import-way/mysql-load-manual) | csv                     |                                       |
| 导入本地数据 |                                                              |                         | 本地文件                              |
| JDBC同步     |                                                              |                         | 通过JDBC导入                          |
| JSON格式     |                                                              |                         | 导入JSON格式数据                      |
