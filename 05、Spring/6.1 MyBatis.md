...
`@Param(value)`用于修饰变量，[主要用于4种场景](https://juejin.cn/post/6844903894997270536)：

1. 方法有多个参数，需要使用`@Param`。
2. 
...
假设`student`表有`id`、`name`、`sex`、`address`等字段，但是SQL中只查询了部分字段，如`id`、`sex`，**MyBatis**在将结果映射成POJO时，就可能会抛出`IndexOutOfBoundsException`以及[上面的异常信息](https://blog.csdn.net/Lu0v0Lu/article/details/119187176)：

解决方法很简单：将全部字段分别列出，或直接使用`*`即可。

> 这一现象不是必然发生的，可能与**MyBatis**的版本，以及了`<select></select>`中使用了`resultType`/`resultMap`有关。
> 
> 再说，就是有只查询部分字段、没必要列出全部字段的场景呀。
