...
- 使用[**Hytrix**](https://www.cnblogs.com/duanxz/p/7519893.html)。

- 使用**Redis**[[2]](https://blog.csdn.net/johnf_nash/article/details/89791808)。

   - 计数器：存储一个键用于计数，并为其设置有效时间，其Lua为：
   ...
   - 前面说过，计数器不够平滑、有临界问题，可使用**Redis**的**List**类型记录<u>访问时间</u>，当**List**长度超过<u>数量阈值</u>时，就判断”最早放入的<u>访问时间</u>“是否小于<u>时间阈值</u>，如果是，则需要限流，否，则将”此次<u>访问时间</u>“放入，同时移除”最早放入的<u>访问时间</u>“：
   ...
   - 令牌桶：查看[文章](https://www.cnblogs.com/duanxz/p/4494072.html)中的11/18处。
...
**Hystrix**使用了设计模式中的<u>命令模式</u>，对外部资源的调用和Fallback都会封装成一个Command。

**Hystrix**[会统计](https://louluan.blog.csdn.net/article/details/90494911)Command执行过程中的<u>成功数</u>、<u>失败数</u>、<u>拒绝数</u>和<u>超时数</u>，并将其记录到断路器（Circuit Breaker）中，断路器会将这些统计信息按照时间窗口的形式记录到一个定长数组中，以此来判断熔断的时机。
...
**Hystrix**使用线程池进行隔离，其优势有：

- 超时返回，避免了同步时，不释放、资源耗尽。
- 服务方可以控制请求数量，多出来的请求可以排队，排队数量超出阈值直接Fallback。

但也有不足：

- 线程数过多，会增加开销，降低吞吐。

所以**Hystrix**还提供基于信号量（Semaphore）的控制线程并发量的隔离方式。Semaphore基于**CAS**，省去了锁竞争、线程池的开销，但本质上是同步的，无法实现超时。
