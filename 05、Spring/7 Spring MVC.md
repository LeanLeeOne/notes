...
1. 可通过`Listener`（`ContextLoaderListener`），有通过`Servlet`（`DisptachServlet`）来创建。
...
## ContextLoaderListener、DispatcherServlet启动顺序

Java Web开发的核心组件：`Listener`、`Filter`、`Servlet`/`JSP`，而各组件的启动顺序为：`Listener` -> `Filter` -> `Servlet`。

受启动顺序的影响，想在`Listener`、`Filter`、`JSP`中如果想获取**Bean**，只能通过如下方式：
...
## ContextLoaderListener、DispatcherServlet配置
...
2. 小于0，或没有指定：表示收到HTTP请求时才会加载该`Servlet`。
