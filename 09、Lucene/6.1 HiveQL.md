...
`row_number() over (partition by user_id order by login_date) as virtual_begin_date`、`min(login_date) over (partition by user_id, real_begin_date) as begin_date`：<span style=background:#c2e2ff>连续</span>登录天数。
