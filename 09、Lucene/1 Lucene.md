...
全文检索是指：将一段文字或一篇文档，先进行分词处理，然后将词和文档建立映射，并将映射组织为索引，之后通过在索引中检索词，就可以找到相应的文章（全文），而建立的索引也被称为倒排索引。
...
**Elasticsearch** + **HBase**的组合，是搜索 + 详情场景的常见解决方案。对于这种“同一份数据保存在不同数据源”的场景，数据的一致性是一个不容忽视的问题。

对此，可以基于<u>消息队列</u>来解决。

- 解耦。
  - 将<u>对不同数据源的写入</u>，由业务系统直接写入的方式，改为基于<u>消息队列</u>间接写入，以实现业务逻辑与同步逻辑的<span style=background:#c2e2ff>解耦</span>。

- 写入。
  - 不同的数据源可以由不同的**Consumer**写入，这样效率高，耦合小，但无法保证一致性。
  - 不同数据源可以由同一**Consumer**来写入，当写入某一数据源失败时，通过不断重试直至成功的方式来保证一致性。
    - 采用并发写入，还是串联写入，对吞吐量的影响不大，并且对不同数据源的处理逻辑都是耦合的，但前者的延迟低，后者更容易实现。
- 核对。
  - 额外自定义一个**Consumer**，该**Consumer**不会立即消费新消息，而是利用<u>消息队列</u>的持久性，延迟一段时间，等各个数据源中的增量数据落盘后，才对各个数据源进行核对（回表），若发现数据不一致，则进行处理。


> 延迟而非实时，一定程度上也是为了减轻各个数据源的IO，避免、缓解IO毛刺。
