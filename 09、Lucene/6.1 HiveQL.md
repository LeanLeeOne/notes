...
> 但反过来则不一定成立：部分Reducer运行时长过长、发生`OutOfMemoryError`，不一定是数据倾斜导致的，比如数据量过大直接导致的运行时间长、内存溢出。
...
- 对于后者，只能且只需缓解。
  ...
  - 基于随机前缀和扩容表的`JOIN`：对分布不均的表增加`N`以内的随机前缀，以令其分布均匀，此时分布均匀的表需要配合前者的变化而扩容`N`倍。适用于异常Key较多的情况。
...
- `max(score)、sum(score) over (order by current_day)`：[截止到当前的](https://www.nowcoder.com/practice/1ce93d5cec5c4243930fc5e8efaaca1e)，单日<span style=background:#c2e2ff>最高</span>成绩、<span style=background:#c2e2ff>累计</span>分数。
...
- `sum(cnt)、max(cnt) over (partition by tag order by start_date rows 6 preceding)`：每类对象[7天内](https://www.nowcoder.com/practice/f90ce4ee521f400db741486209914a11)的累计量、最大值。
...
- `sum(if_follow=[1,-1]) over (partition by i.author order by l.start_time) as follow_count`：每个作者截止到当前时间的累计粉丝数。
...
- `lag(date, 1)/lead(date, 1) over (partition by uid order by date)`：某用户某项行为的<span style=background:#c2e2ff>时间间隔</span>。
