...
**Kafka**为了追求高吞吐，在内存、磁盘、网络等3个方面做了很多设计，包括顺序读写、分区、支持压缩、支持批量、数据结构（索引）、NIO+多路复用，以及<span style=background:#c9ccff>Page Cache</span>、**MMap**、**Zero Copy**等。
...
> 在64位操作系统中一般可以表示20G的数据文件，`20GB`的内存按`50MB/S`的速度写入，[至少可以](https://blog.csdn.net/suifeng3051/article/details/48053965#52-集群大小)缓存`7分钟`的消息。
