...
**Bully**通过在**Follower**间发送消息来选举ID最大（也可以是最小）的节点为**Leader**。

**Follower**通过心跳发现集群拓扑发生变化时，会发起选举：

1. 当前**Follower**会向所有比自己的ID大的**Follower**发送**Election**，开始选举。
2. 如果额定时间内，有**Follower**向该**Follower**回复**Alive**，表示存活，该**Follower**放弃参选；
3. 否则，该**Follower**当选，并向所有**Follower**发送**Victory**，**Follower**成为**Leader**。

**Follower**会请求其他**Follower**来帮助判断**Leader**是否为假死，如果如果**Leader**是假死，则不发起选举。

由于网络问题，**Bully**可能会选出多个**Leader**，即发生脑裂，可通过要求“在额定时间内获得<span style=background:#ffb8b8>半数以上</span>的选票才能当选，否则集群不可用”的方式来解决。
