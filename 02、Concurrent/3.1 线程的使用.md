...
`ThreadLocal`不是真正存储数据的地方，其内部类`ThreadLocalMap`才是；`ThreadLocalMap`是<u>线程</u>私有的，以`ThreadLocal`作为Key，而非`Thread`，以便**GC**。
...
如：数据库连接、`IO`连接的对象没有关闭，`ThreadLocal`使用完后没有调用`remove()`来将Key为`null`的<u>键值对</u>删掉。
