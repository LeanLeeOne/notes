...
Flink支持YARN、独立模式、Mesos和Kubernetes等运行模式。
...
## 窗口[[1]](https://help.aliyun.com/zh/flink/realtime-flink/developer-reference/window-functions-1/)

**Flink SQL**支持2种窗口聚合：Window聚合和Over聚合。

- Window聚合又分为3种：<u>滑动窗口</u>（HOP）、<u>滚动窗口</u>（TUMBLE）和<u>会话窗口</u>（SESSION）。
- OVER聚合就是标准意义上的开窗，详见6.1 HiveQL中的[窗口函数](./6.1%20HiveQL#窗口函数56⭐)，此处不做赘述。

### 时间属性

Window聚合支持2种时间属性：Event Time和Processing Time。

- Event Time，事件时间，由开发者提指定，通常指定数据的最原始的创建时间。**Flink**会根据Event Time生成Watermark来进行关窗，即只有当Watermark大于关窗时间时，才会触发窗口的结束，才会输出结果。如果一直没有数据来触发窗口结束，则该窗口就无法输出结果。单个subtask的Watermark是递增的，多个subtask或者多个源表的Watermark取最小值。可以通过配置超时参数来触发窗口结束。

- Processing Time，对事件进行处理的本地系统时间，由**Flink**产生，不包含在开发者的原始数据中。
...
1. `time-attr`，时间字段。
2. `slide-interval`，移动的步长/时间间隔/更新周期，即连续窗口之间的时间差。
3. `size-interval`，窗口的大小/持续时间/覆盖范围。

~~对于<u>滑动窗口</u>，**Flink**会将一个事件复制到每个相关的Window，如，每隔`15`分钟生成一个`24`小时的<u>滑动窗口</u>，则每个事件将被复制`4 * 24 = 96`个Window中。~~存疑

<img src="../images/9/flink_sliding_window.svg" style="zoom:50%;" />

<u>滑动窗口</u>根据步长和大小的不同，分为3种：

- 步长小于大小，则窗口会重叠，每个元素会被分配到多个窗口。
- 步长等于大小，则等同于<u>滚动窗口</u>。
- 步长大于大小，则为跳跃窗口，窗口之间不重叠且有间隙。

> <u>滑动窗口</u>也被称作Sliding Window
>
> 关于Window的具体实现，可阅读[文章](http://wuchong.me/blog/2016/05/25/flink-internals-window-mechanism/#Window-的实现)。

### 滚动窗口，TUMBLE

<u>滚动窗口</u>会和时间自动对齐，如，在`12:05`启动了一个`1`小时的Window，那么Window将在`13:00`关闭，而非`13:05`。但**Flink**也允许通过调整参数来改变Window的对齐方式。

### 会话窗口，SESSION

<u>会话窗口</u>没有固定大小，窗口之间没有重叠，相反，当它在一个固定的时间周期内不再收到元素时，会话就会断开，窗口就会关闭。
...
[状态](https://help.aliyun.com/zh/flink/realtime-flink/use-cases/performance-tuning-for-large-state-deployments/#b37022e32008y)可以被视为算子的记忆，它使得算子能够在处理无界流数据时保持对历史数据的跟踪。状态可以是简单的<u>键值对</u>，也可以是更复杂的数据结构，如列表、集合或自定义对象。
...
## 性能调优[[2]](https://nightlies.apache.org/flink/flink-docs-stable/zh/docs/dev/table/tuning/)

**Flink**的Table API和SQL是高效优化过的，集成了很多查询优化和算子优化。但并不是所有的优化都是默认开启的，因此对于某些工作负载，可以通过打开某些选项来提高性能。

### Mini Batch聚合

无界聚合算子是逐条处理输入的记录，当数据量比较大时，算子会频繁地读写状态，StateBackend的开销（尤其是RocksDB StateBackend）会凸显出来。此时如果再有数据倾斜发生，很容易导致Job发生反压。这时可以使用Mini Batch。

Mini Batch其实就是微批，算子将一组输入的记录缓存在内部的缓冲区中，按批读写状态，减少开销，增大吞吐，但会增加延迟。

### Local Global聚合

面对部分数据倾斜问题时，可以采用本地预聚合（Local）后，再全局聚合（Global）的方式来解决。

> 本地预聚合的基础是Mini Batch聚合。

### 拆分去重计数

Local Global聚合可以解决常规聚合的数据倾斜，例如`SUM`、`COUNT`、`MAX`、`MIN`、`AVG`，但不能解决Key<u>分布稀疏</u>时的去重计数`COUNT(DISTINCT 分布稀疏的Key)`，因为此场景下，Local输出的数据量接近输入时的，所以Global需要频繁地读写状态。

可以在开启Local Global的基础上，改写成加盐的SQL来解决。
...
FROM    (
            ...
            FROM    T
            GROUP BY day
                     ,MOD(HASH_CODE(user_id),1024) -- BUCKET_NUM的默认值是1024，故此处采用1024
        ) 
GROUP BY day
;
```
...
`FILTER`在语义上虽然与`CASE WHEN`相同，但在Key相同但过滤条件不同的多个去重计数的场景下，Flink会针对前者，使用一个共享状态实例，而非多个，这有助于减少状态读写和状态大小，进而减少开销。

### Mini Batch Regular Join

Regular Join算子是逐条处理输入的记录，当数据量比较大时，与无界聚合算子会面临同样的问题，此外还会有中间结果放大的问题。这时可以使用Mini Batch。

### Delta Join

Regular Join会维护来自两个输入的所有历史数据，以确保结果的准确性。随着时间的推移，状态会不断增长，资源会不断消耗，影响作业的稳定性。对此，**Flink**引入了Delta Join算子。

Delta Join的核心思想是基于双向Lookup Join来替代Regular Join所维护的大状态。所谓的Lookup Join就是，直接查询外部源表中的数据。

### 其它[[3]](https://help.aliyun.com/zh/flink/realtime-flink/use-cases/control-state-size-to-prevent-backpressure-in-sql-deployments)

除了以上场景跟措施，还有：

1. [尽可能减少读盘](https://help.aliyun.com/zh/flink/realtime-flink/use-cases/control-state-size-to-prevent-backpressure-using-the-datastream-api#8acb6652ebs5v)。
   1. 分配更多内存。
   2. 增加并发数，进而降低单个并发任务的状态量。
2. 主动避免生成不必要的状态算子，比如ChangelogNormalize、SinkUpsertMaterializer等算子。
3. 减小状态大小。
   1. 设置合理生命周期。
   2. 命中更优的执行计划：利用主键优化双流连接、优化append_only流去重操作（也就是使用`ROW_NUMBER()`替代`FIRST_VALUE()`或`LAST_VALUE()`）和上文的使用FILTER子句。
   3. 调整多流Join顺序（小表先关联)，缓解State放大。

**Flink**并没有明确的“大状态”定义，但通常状态大小超过`1GB`或状态数量超过`1000万条`属于“大状态”。

反压可能由多种因素引起，但最主要的原因之一是Job状态的持续膨胀，直至超出内存限制。此时，State Backend会将部分不频繁使用的状态移至磁盘，磁盘读写速度远慢于内存，效率瓶颈由此产生。
