...
一个服务往往会部署多个实例，为了防止因不健康实例过多导致流量全部流向健康实例，继而造成健康实例的流量压力陡增并被压垮进而形成雪崩效应，应设定一个值为`0~1`的浮点数作为阈值，当健康实例数占服务实例总数的比例小于阈值时，无论实例是否健康，都会将实例返回给Consumer。这样虽然会损失一部分流量，但是保护了服务集群中剩余的健康实例。
