...
`System.out.println()`、`e.printStackTrace()`可以向控制台中输出异常等信息，但有一定的局限性；但Java提供**日志**输出，令我们轻松便捷地输出异常等信息。
...
**Log4j API**作为Facade，可以搭配**Log4j**实现和其它日志实现使用，如Logback。
...
2. 支持Lambda表达式。
3. 提供的日志记录方法更多。
4. 除了SLF4J支持的“参数化日志”格式之外，Log4j API还支持使用`java.text.MessageFormat`语法的事件以及printf样式的消息。
